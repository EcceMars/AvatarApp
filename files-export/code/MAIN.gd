...
func _get_listener_script() -> String:
	...
	# For exported app, check both with and without .py extension
	var base_dir = OS.get_executable_path().get_base_dir()
	var script_path = base_dir.path_join("keyboard_listener.py")
	
	# Try with .py extension first (since that's what you copied)
	if FileAccess.file_exists(script_path):
		return script_path
	
	# Fallback to without extension
	script_path = base_dir.path_join("keyboard_listener")
	if FileAccess.file_exists(script_path):
		return script_path
...
func _start_keyboard_listener()->void:
	...
	#if not script:
		#push_error("COULD NOT FIND THE KEYBOARD_LISTENER")
		#get_tree().quit(1)
...
func _apply_scale(_direction:float)->void:
	current_scale = clampf(current_scale + STEP_SCALE * sign(_direction), MIN_SCALE, MAX_SCALE)
	...
	if mac:
		...
		AVATAR_WINDOW.position.y -= y_off * sign(_direction)
		AVATAR_WINDOW.position.x -= x_off * sign(_direction)
